...
class Buffer:
    ...
    def __init__(self, pv_list: list[str], buffer_len: int, logging_kwargs: Optional[dict] = default_logging_kwargs):
        ...
        self.data_map = {pv: SlidingWindowArray(buffer_len, dtype=np.float64, logging_kwargs=logging_kwargs.copy()) for pv in self.pv_list}
        ...
        self.data_map["pv_timestamps_ns"] = SlidingWindowArray(buffer_len, dtype=np.float64, logging_kwargs=logging_kwargs.copy())
        self.data_map["beam_checks"] = SlidingWindowArray(buffer_len, dtype=bool, logging_kwargs=logging_kwargs.copy())
        self.data_map["bpm_score_1"] = SlidingWindowArray(buffer_len, dtype=np.float64, logging_kwargs=logging_kwargs.copy())
        self.data_map["bpm_score_20"] = SlidingWindowArray(buffer_len, dtype=np.float64, logging_kwargs=logging_kwargs.copy())
