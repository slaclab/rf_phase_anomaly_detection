...
from beam_check_config import NANOSECS_IN_1_SEC, SAMPLES_PER_SECOND, SAMPLES_PER_SECOND, NANOSECS_IN_1_SEC
...
def test_buffer_update_basic(pv_list):
    ...
    snapshot_1 = {
        pv: [make_entry(ts, float(i)) for i, ts in enumerate(timestamps_1)]
        for pv in pv_list
    }

    snapshot_2 = {
        pv: [make_entry(ts, float(i + 1000)) for i, ts in enumerate(timestamps_2)]
        for pv in pv_list
    }
