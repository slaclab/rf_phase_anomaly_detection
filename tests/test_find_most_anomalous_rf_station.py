...
from beam_check_config import RF_PV_NAMES, CANDIDATE_LOOKBACK_WINDOW_LENGTH, FEEDBACK_STATIONS
...
def test_on_random_data(make_random_data):
    rf_name, deviation, system = find_most_anomalous_rf_station(make_random_data)
    assert system  # should be true


def test_on_single_anomaly_data(make_single_anomaly_data):
    index, data = make_single_anomaly_data
    rf_name, deviation, system = find_most_anomalous_rf_station(data)
