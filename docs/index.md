# RF Phase Anomaly Detection

For now, these docs just show some diagrams produced by the team..

This diagram describes in detail the steps taken in Process B to generate candiates.
![Detailed Diagram](./imgs/detailed_process_b_visual.png)
(credit: https://github.com/bhardwaj-gopika)

This diagram shows at a high-level how to data flows betwen the processes and the shared buffers.
![Pipeline Diagram](./imgs/high_level_pipeline_visual.png)
(credit: https://github.com/nstelter-slac)
