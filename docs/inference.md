The `/inference` subdirectory contains the inference models used to detect the anomalies.
It contains the models and necessary files/modules to run inference on the model.

## Model Files
The model files are stored in the `/inference/models` subdirectory. The model files include
...
placed in the `/inference/models` subdirectory and that the YAML files are updated accordingly.
